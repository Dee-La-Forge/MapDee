...
| ~~**B7**~~ | ~~**« mur », « contact », « bande d'étude » ne sont définis nulle part**~~ — **LEVÉ le 06/08** : C2 trois passes (protocoles pré-enregistrés), ADR-009 (prix exécutés) + ADR-010 (variante exécution) **ACCEPTÉES**. Mur : M′ = 423,5/175,4, P′ = 1,5 s/1 s (voisinage relatif ±0,05 %). Contact : première transaction au palier. Bande : ±0,0925 %/±0,1451 %. Les trois EN BLOC dans `03` avec sources | A2 et B4 deviennent calculables ; la chaîne É3/É4 n'attend plus que rejeu, D12, gel de C3 |
...
| **garde de couverture dans `construit/`** — la garde est binaire (0 photo = refus, >0 = compté) : 20251213 BTC est passé à 172 photos, 20251215 à 44-54 %. Manifeste : ajouter `heures_a_zero_photo` ; `lot.py` : refus sous plancher ; préflight : refuser un périmètre à jour sous plancher pour les épreuves à l'unité JOUR. Valeur du plancher = ADR-007 (chez Meddy) | table de couverture du 06/08 (`journal/2026-08-06_couverture-j8-*.md`) | même fenêtre que les corrections du lanceur — jamais sous un run |
| **contrôle jour-par-jour d'É2** (recommandation d'audit du 06/08) : recalculer le ρmax d'É2 PAR JOUR pour les 9 candidats jugés — confirme ou infirme des verdicts déjà écrits pour le coût d'un recalcul ; stabilité inter-jours = robustesse, instabilité = drapeau | audit externe du 06/08, point 3 | quand une main est libre, après la 2ᵉ passe C2 |
| **garde de chauffe** (la contagion 12→13 ne doit plus être possible silencieusement) : à la première photo émise, si le carnet hérité de la chauffe est croisé, REFUS du jour avec raison — à implémenter dans la fenêtre C10, pas avant (le mode sans-veille du 13 est hors de sa portée) | anatomie de l'incident du 06/08 | liste C10, étape corrections `construit/` |
| **audit `construit/` du 06/08 — trois corrections d'observabilité** : (1) scinder `n_temps_approx` en `n_updates` / `n_sans_temps` (le compteur de jointure est saturé par les updates, ~12 M/jour) ; (2) extraction atomique dans `_extract_day` (tmp + rename — le `.gz` tronqué passe encore « déjà extrait », l'EOFError du 05/08 jamais corrigé en code) ; (3) compteur `n_illisibles` dans `read_diffs` (`except: continue` muet). Plus la **piste de récupération 12/13/15** : rejeu diagnostic qui logge les oids du haut de carnet à l'entrée en croisement — si un petit ensemble d'ordres fantômes épingle le carnet, une éviction par ADR pourrait récupérer 2-4 jour-symboles | `journal/2026-08-06_audit-construit.md` | même fenêtre — jamais sous un run |
...
| **C6 (incertitude) n'est pas spécifié** — et `06` §9 interdit la tranche 2 avant lui ; l'oublier figerait la réserve par défaut. **PRÉ-DÉCLARÉ le 06/08, avant toute mesure de C6 : σ inter-journalier se mesure sur les jours PLEINS uniquement** (la liste nommée de la table de couverture du 06/08 ; plancher formel = ADR-007 une fois arbitrée). Quatre jours dégradés (12 et 15, ≈44-75 %, amputés de leurs heures actives) mesurés dedans donneraient le σ d'une unité hétérogène — pas la quantité qu'`ADR-000` nomme | nulle part avant cette table + table de couverture du 06/08 | spécifier C6 avant la fin de tranche 1 + arbitrage tranche 2 |
